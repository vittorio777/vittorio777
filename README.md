## Hello there 👋

<img align="right" width="260" src="./giphy.gif">

I'm a software developer interested in building complete products,  
from the interface to the backend and everything in between.

- ⚙️ Daily drivers: `.cs` `.ts` `.py` `.sql`
- 🧰 Building with `.NET` `React` `PostgreSQL` `Docker`
- 🔬 Machine learning, research, and experiments
- 💻 I make computers do things. Occasionally useful things.

<br>

<img height="32" src="https://skillicons.dev/icons?i=cs,dotnet,ts,react,python,pytorch,postgres,docker,azure,git">

<br clear="right">
